...
public class CustomAutowireConfigurer implements BeanFactoryPostProcessor, BeanClassLoaderAware, Ordered {
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		if (this.customQualifierTypes != null) {
			...
			// 如果dlbf中的用于检查备案定义的autowire选候者解析器不是QualifierAnnotationAutowireCandidateResolver子类，那么
			...
			// 遍历自定义限定符类型(Class<? exteds Annotation>或者String类型)
			for (Object value : this.customQualifierTypes) {
				Class<? extends Annotation> customType = null;
				if (value instanceof Class) {
					// 如果是Class类型，那么强制转化为Class<? exteds Annotation>类型
				...
				// 将customType添加到用于检查bean定义是否为autowire候选者的解析其中
